...
- Markdown files do not make a good database for a multi-user system. The second you have more than one user or process running, re-writing a file becomes a potentially destructive action. Entries in a db should be atomic, so writes don't risk overwriting unrelated changes.
